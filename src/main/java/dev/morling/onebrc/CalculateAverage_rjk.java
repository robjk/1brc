...
public class CalculateAverage_rjk {
    ...
    public static final int MAP_SIZE = 65536; // somewhat arbitraily chosen to reduce collisions
    public static final int POSITION_MASK = MAP_SIZE - 1;


    // Total elapsed times are not riguorous, methodology is to run repeatedly on a windows machine and take somewhere
    // in the middle of the range for the table below.  It's been fine so far but improvements the error-bars are
    // getting to the stage where a more serious approach to timing will be necessary.
    ...
    private static class StationResult {
        ...
        public StationResult(byte[] bytes, int length, int hash, int measurement) {
        ...
        public boolean equals(byte[] nameBuffer, int otherLength, int otherHash) {
            if (bytes.length == otherLength && hash == otherHash) {
        ...
        // lingering around to aid debugging, ultimately will disappear
    ...
    private static class LinearProbingHashMap {
         ...
         * linear probe looks to be a good fit.  Simple to implement and as long as occupancy is kept low then fairly
         * performant.
         */
        StationResult[] table = new StationResult[MAP_SIZE];

        public void upsert(byte[] bytes, int length, int hash, int measurement) {
            int position = hash & POSITION_MASK;

            while (true) {
                StationResult entry = table[position];
                if (entry == null) {
                    // empty slot, create a new entry and we're done
                    table[position] = new StationResult(bytes, length, hash, measurement);
                    return;
                }

                // found matching entry, update it
                if (entry.equals(bytes, length, hash)) {
        ...
        public void upsert(StationResult update) {
            ...
            while (true) {
                ...
                if (entry.hash == update.hash) {
                    if (entry.bytes.length == update.bytes.length) {
                        for (int i = 0; i < entry.bytes.length; i++) {
                            if (entry.bytes[i] != update.bytes[i]) {
                                position = (position + 1) & POSITION_MASK;
                                continue next_entry;
                            }
                        }
        ...
        public void combine(LinearProbingHashMap otherMap) {
    ...
    private static class Worker implements Callable<LinearProbingHashMap> {
            ...
            try {
                ...
                while (true) {
                    ...
                    if (lastChar == ';') {
                        ...
                        if (entry != null && entry.equals(bytes, length, hash)) {
                            entry.recordMeasurement(measurement);
                        } else {
                            // quick lookup failed, take the slow path
                            stationResults.upsert(bytes, length, hash, measurement);
                        }
                        ...
                        // collect more of the name
                        bytes[length++] = lastChar;
                        hash ^= 11587 * lastChar;       // prime chosen by "feels"/empirically counting collisions
                    }
                }
            } catch (Exception e) {
        ...
        private int extractMeasurement(MemorySegment source) {
            ...
            if ((measurementInt & 0xFF000000) == NEW_LINE_MSB) {
                ...
                scaledInt = (measurementInt & 0xF) * 10
                        + (measurementInt >> 16 & 0xF);
                sourceOffset += 4;
            } else {
                // 2 integer digits     xx.y\n
                scaledInt = (measurementInt & 0xf) * 100
                        + (measurementInt >> 8 & 0xF) * 10
                        + (measurementInt >> 24 & 0xF);
                sourceOffset += 5;
            }

            return scaledInt * negativeMultiplier;
        }
    }
    ...
    void main() {
        ...
        try (FileChannel channel = FileChannel.open(Path.of("D:\\development\\workspace\\1brc\\measurements.txt"), StandardOpenOption.READ)) {
            ...
            futures[THREAD_COUNT - 1] = startWorker(source, startingOffset, source.byteSize());
