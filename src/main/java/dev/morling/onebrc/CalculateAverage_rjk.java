...
public class CalculateAverage_rjk {
    ...
    public static class Timer {
        ...
        public String toString() {
            ...
            return String.valueOf(TimeUnit.NANOSECONDS.toMillis(end - start));
        }
    }

    /**
     * First step, read the file and learn about the data (can just read the generator code, but this dips a toe in)
     */
    private static class Analysis {

        public Analysis(MemorySegment source) {
            // 35,939ms
            Timer timer = new Timer();

            System.out.println("total bytes: " + source.byteSize());

            long lineCount = 0;
            long maxCityLength = Long.MIN_VALUE;
            long minCityLength = Long.MAX_VALUE;
            ...
            while (offset < source.byteSize()) {
                while ((lastChar = source.get(JAVA_BYTE, offset++)) != (byte) '\n') {

                    if (lastChar == ';') {
                        long cityLength = offset - startOfCity - 1;
                        maxCityLength = Math.max(maxCityLength, cityLength);
                        long oldMinCityLength = minCityLength;
                        minCityLength = Math.min(minCityLength, cityLength);
                        if (minCityLength != oldMinCityLength) {
                            System.out.println("minCityLength: " + minCityLength + " for " + new String(source.asSlice(startOfCity, cityLength).toArray(JAVA_BYTE), StandardCharsets.UTF_8));
                        }
                    }
                }
                ...
                offset++;   // advance beyond the \n
            }

            System.out.println("\nelapsed=" + timer + "ms, lines=" + lineCount + ", city lengths: min=" + minCityLength + ", max=" + maxCityLength);
        }
    }

    /**
     * First attempt, ensure we've not gone wildly off-piste with assumption offHeap reading is going to be ok
     */
    private static class MemoryMappedSingleThreadNoOptimisations {
        ...
        public void compute(MemorySegment source) {
            long offset = 0;
            byte lastChar;
            long startOfCity = 0;
            ...
            while (offset < source.byteSize() && (lastChar = source.get(JAVA_BYTE, offset++)) != (byte) '\n') {
                ...
                if (lastChar == ';') {
                    ...
                    long startOfMeasurement = offset;
                    while ((lastChar = source.get(JAVA_BYTE, offset++)) != (byte) '\n') {
                        // future improvements go here
                    }

                    long measurementLength = offset - startOfMeasurement;
                    byte[] measurementBytes = source.asSlice(startOfMeasurement, measurementLength).toArray(JAVA_BYTE);
                    double measurement = Double.parseDouble(new String(measurementBytes, StandardCharsets.UTF_8));
            ...
            String output = stats.values().stream()
    ...
    void main() {
        ...
        try (FileChannel channel = FileChannel.open(Path.of("D:\\development\\workspace\\1brc\\measurements.txt"), StandardOpenOption.READ);
            ...
            MemoryMappedSingleThreadNoOptimisations memoryMappedSingleThreadNoOptimisations = new MemoryMappedSingleThreadNoOptimisations();
            memoryMappedSingleThreadNoOptimisations.compute(source);
